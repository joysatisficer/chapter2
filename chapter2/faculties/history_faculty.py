...
async def history_faculty(history: ActionHistory, faculty_config: HistoryFacultyConfig, em: EmConfig):
    ...
    for message in reversed(messages):
        if faculty_config.nickname is not None and message.author.name == faculty_config.nickname:
            yield Message(Author(em.name), message.content, type=message.type)
        elif message.author.name in faculty_config.nicknames:
            yield Message(Author(faculty_config.nicknames[message.author.name]), message.content, type=message.type)
        else:
            yield message
